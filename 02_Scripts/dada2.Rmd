...
# ── Distribución de lecturas por muestra (16S) ─────────────────────────────
ss16S   <- sample_sums(ps16S)
meta16S <- as.data.frame(sample_data(ps16S))
...
df_reads <- merge(
  df_reads,
  meta16S[, "Sample.Name", drop = FALSE],
  by.x  = "Sample",
  by.y  = "row.names",
  all.x = TRUE
)

df_reads$Genotipo <- sub("^[0-9]+_", "", df_reads$Sample.Name)

genotipos <- sort(unique(df_reads$Genotipo))
n_gen     <- length(genotipos)
colores   <- setNames(
  colorRampPalette(brewer.pal(min(n_gen, 11), "Set3"))(n_gen),
  genotipos
)

n_samples <- nrow(df_reads)

ggplot(df_reads, aes(x = reorder(Sample.Name, TotalReads),
                     y = TotalReads,
                     fill = Genotipo)) +
...
# Save at a tall resolution so every label is legible
ggsave("03_Results/figures/16S/reads_por_muestra_16S.png",
       width = 10, height = 22, dpi = 200)
